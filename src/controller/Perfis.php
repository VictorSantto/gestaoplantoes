<?php
	...
	if ($method == "page") {
		...
		echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . 
				(isset($_GET["friendly"]) ? "/html/perfis.html" : "/html/perfis.html"), $response);
	 ...
	 */
		...
		$result = true;
		$daoFactory->beginTransaction();
		$call = $daoFactory->getPerfisDao()->read("perfis.id = " . $form[0], "", false);
		$answer = $daoFactory->getPerfisDao()->read("perfis.id = " . $form[1], "", false);
		$perfisDao = $daoFactory->getPerfisDao()->read("perfis.ordem >= " . $answer[0]["perfis.ordem"], "", false);
		if (is_array($perfisDao) && sizeof($perfisDao) > 0) {
			for ($x = 0; $x < sizeof($perfisDao); $x++) {
				$perfis = new model\Perfis();
				$perfis->setId($perfisDao[$x]["perfis.id"]);
				$perfis->setPerfil(logicNull($perfisDao[$x]["perfis.perfil"]));
			$perfis->setCadastrado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$perfis->setModificado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			...
			$perfis = new model\Perfis();
			$perfis->setId($call[0]["perfis.id"]);
			$perfis->setPerfil(logicNull($call[0]["perfis.perfil"]));
			$perfis->setCadastrado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$perfis->setModificado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
