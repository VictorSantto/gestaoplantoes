<?php
	...
	if ($order != "") {
		...
		if ($method == "stateReadAll" || $method == "stateCalledAll") {
			$limit = "situacoes_registros.ordem ASC";	
		} else {
	...
	if ($method == "page") {
		...
		echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . 
				(isset($_GET["friendly"]) ? "/html/situacoes_registros.html" : "/html/situacoes_registros.html"), $response);
	 ...
	 */
		...
		$result = true;
		$daoFactory->beginTransaction();
		$call = $daoFactory->getSituacoes_registrosDao()->read("situacoes_registros.id = " . $form[0], "", false);
		$answer = $daoFactory->getSituacoes_registrosDao()->read("situacoes_registros.id = " . $form[1], "", false);
		$situacoes_registrosDao = $daoFactory->getSituacoes_registrosDao()->read("situacoes_registros.ordem >= " . $answer[0]["situacoes_registros.ordem"], "", false);
		if (is_array($situacoes_registrosDao) && sizeof($situacoes_registrosDao) > 0) {
			for ($x = 0; $x < sizeof($situacoes_registrosDao); $x++) {
				$situacoes_registros = new model\Situacoes_registros();
				$situacoes_registros->setId($situacoes_registrosDao[$x]["situacoes_registros.id"]);
				$situacoes_registros->setSituacao_registro(logicNull($situacoes_registrosDao[$x]["situacoes_registros.situacao_registro"]));
			$situacoes_registros->setCadastrado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$situacoes_registros->setModificado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$situacoes_registros->setCor($situacoes_registrosDao[$x]["situacoes_registros.cor"]);
			
				$resultDao = $daoFactory->getSituacoes_registrosDao()->update($situacoes_registros);			
				$result = !$result ? false : (!$resultDao ? false : true);
			}
			$situacoes_registros = new model\Situacoes_registros();
			$situacoes_registros->setId($call[0]["situacoes_registros.id"]);
			$situacoes_registros->setSituacao_registro(logicNull($call[0]["situacoes_registros.situacao_registro"]));
			$situacoes_registros->setCadastrado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$situacoes_registros->setModificado(date("Y-m-d H:i:s", (time() - 3600 * 3)));
			$situacoes_registros->setCor($call[0]["situacoes_registros.cor"]);
		...
		if (!getActiveSession($_ROOT . $_MODULE)) 
			 ...
			 */
				...
				if (!getPermission($_ROOT . $_MODULE, $daoFactory, $screen, $method))
					...
					$response["situacoes_registros"][0]["situacoes_registros.cores"] = $daoFactory->getCoresDao()->read("cores.tipo_cor = 1", "cores.id ASC", false);
