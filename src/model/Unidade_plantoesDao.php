<?php
	...
	class Unidade_plantoesDao {
		 ...
		 * Constant variables
		 */
		private $create = "INSERT INTO unidade_plantoes (
				data
				, cadastrado
				, modificado
				, unidade
		...
		public $read = 
				"unidade_plantoes.id AS \"unidade_plantoes.id\"
				, unidade_plantoes.data AS \"unidade_plantoes.data\"
				, unidade_plantoes.cadastrado AS \"unidade_plantoes.cadastrado\"
				, unidade_plantoes.modificado AS \"unidade_plantoes.modificado\"
				, unidade_plantoes.unidade AS \"unidade_plantoes.unidade\"
				";
				
		private $update = "UPDATE unidade_plantoes SET";
		private $delete = "DELETE FROM unidade_plantoes";
		
		public $from = "unidade_plantoes unidade_plantoes";
		...
		/**
		 * @param {Unidade_plantoes}unidade_plantoes
		 */
		public function setCreate($unidade_plantoes) {		
			$this->sql = $this->create . " (\"" . 
					$unidade_plantoes->getData() .
					"\", \"" . $unidade_plantoes->getCadastrado() .
					"\", \"" . $unidade_plantoes->getModificado() .
					"\", \"" . $unidade_plantoes->getUnidade() .
					"\")";
		}
		...
		public function setRead($where, $order) {
			...
			$this->sql = "SELECT " . $this->read . ", " . $unidadesDao->read . 
					...
					($this->getWhere() == "" ? " WHERE unidade_plantoes.unidade = unidades.id" : $this->getWhere()) . 
					" AND unidade_plantoes.unidade = unidades.id" . $this->getOrder();
		}
		 ...
		 * @param {String} where
		 */
		public function setUpdate($unidade_plantoes, $where) {
			$this->setWhere($where);
			
			$this->sql = $this->update . 
					" id = \"" . $unidade_plantoes->getId() . 
					"\", data = \"" . $unidade_plantoes->getData() . 
					"\", modificado = \"" . $unidade_plantoes->getModificado() . 
					"\", unidade = \"" . $unidade_plantoes->getUnidade() . 
					"\"" . $this->getWhere();
		}
		...
		private function setSize($where) {
			...
			$result = $this->connection->execute(
					"SELECT count(1) AS \"unidade_plantoes.size\" from unidade_plantoes" . $this->getWhere());

			while ($row = $result->fetch_assoc()) {		
				$this->setResponse(0, "unidade_plantoes.size", $row["unidade_plantoes.size"]);
				
				$pages = ceil($row["unidade_plantoes.size"] / $this->connection->getItensPerPage());
				
				$this->setResponse(0, "unidade_plantoes.page", $this->connection->getPosition());
				$this->setResponse(0, "unidade_plantoes.pages", $pages);
				...
				$this->setResponse(0, "unidade_plantoes.pagination", $pagination);
			}
		 ...
		 * @return {Boolean}
		 */
		public function create($unidade_plantoes) {
			$result = "";

			$this->setCreate($unidade_plantoes);
		...
		public function read($where, $order, $wp) {
			...
			while ($row = $result->fetch_assoc()) {
				...
				$this->setResponse($line, "unidade_plantoes.line", $line);
		 ...
		 * @return {Boolean}
		 */
		public function update($unidade_plantoes) {
			$result = "";
			
			$this->setUpdate($unidade_plantoes, "unidade_plantoes.id = " . $unidade_plantoes->getId());
		...
		public function combo($selected, $order) {
			...
			while ($row = $result->fetch_assoc()) {
				$this->setResponse($size, "unidade_plantoes.id", $row["unidade_plantoes.id"]);
				$this->setResponse($size, "unidade_plantoes.data", $row["unidade_plantoes.data"]);
			
				if ($row["unidade_plantoes.id"] == $selected)
					$this->setResponse($size, "unidade_plantoes.selected", "selected");
				else
					$this->setResponse($size, "unidade_plantoes.selected", "");
