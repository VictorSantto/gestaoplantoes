<?php
	...
	if ($method == "page") {
		...
		if (is_array($response["usuarios"]) && sizeof($response["usuarios"]) > 0) {
			...
			echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . "/html/side.html", $response);
			echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . "/html/minha_conta.html", $response);			
		...
		if ($method == "update") {
			...
			$response["titles"] = $daoFactory->getTelasDao()->read("telas.identificador = \"" . $screen . "\"", "", 
					true);
			$response["usuarios"] = $daoFactory->getUsuariosDao()->read("usuarios.id = " . 
			...
			echo $view->parse($_DOCUMENT_ROOT . $_PACKAGE . "/html/minha_conta/minha_contaCST.html", $response);
		}
	}
